...
get_england <- function(d, two_englands = FALSE) {
  if (two_englands) {
    d <-
      d %>%
      filter(str_detect(toupper(Name), "ENGLAND \\(ALL ACUTE TRUSTS\\)"))
  } else {
    d <-
      d %>%
      filter(str_detect(toupper(Name), "ENGLAND"))
  }
...
# Set all dates to be in one year so this historical data overlays the 2022-23 data on the graphs
england <-
  england %>%
  mutate(
    year = case_when(
      Date >= dmy("01-11-2022") & Date <= dmy("01-05-2023") ~ "2022-23",
    ...
    day_of_year = if_else(month(Date) >= 11,
                          ...
                          as.Date(paste("2023", month(Date), mday(Date), sep = "-")))
  )
...
beds_england_current <- beds_england %>%
  filter(year == "2022-23")

beds_england_previous <- beds_england %>%
  filter(year == "2021-22")

beds_england_historical <- beds_england %>%
  filter(!year %in% c("2022-23", "2021-22")) %>%
  group_by(name, day_of_year) %>%
  summarise(value = median(value)) %>%
  ungroup() %>%
  mutate(year = "2015-16 to 2020-21")

beds_england <- bind_rows(
  beds_england_current,
  beds_england_previous,
...
england_summary <-
  ...
  filter(
    ...
    year %in% c("2022-23", "2021-22", "2020-21", "2019-20")
  ) %>%
...
beds_trusts_current <- beds_trusts %>%
  filter(year == "2022-23")

beds_trusts_previous <- beds_trusts %>%
  filter(year == "2021-22")
...
trusts_geocoded <-
  ...
  right_join(
    trusts |> filter(Date >= ymd("2022-11-01") & Date <= ymd("2023-05-01")),
